...
int main() {
	...
	//cube1,cilinder1,pie2,rectangle1
	targets.push_back("cube1");
	targets.push_back("cilinder1");
	targets.push_back("rectangle1");
	targets.push_back("pie2");
	// This algorithm tries to get the most collective ERs possible
	// First, it tries the 4 collective ER
	// Then, 1 collective (of 3 elements) and 1 singular
	// Then, 2 collective (of 2 elements)
	// Then, collective (of 2 elements) and 2 singular
	// Then, 4 singular
	int cases = 0;
	set <string> entities;
	...
	while (getline(cin, currentline)) {
		...
		if (currentline.find("--") != -1) {
			...
			// First of all, I want to see if I have 4-collective ERs
			...
			if (doubles.size() > 0 && !answerfound) {
				...
				if (singulars.size() == 4) {
					string er1 = singulars[targets[0]];
					string er2 = singulars[targets[1]];
					string er3 = singulars[targets[2]];
					string er4 = singulars[targets[3]];
					string ans =  er1 + " && " + er2 + " && " + er3 + " && " + er4;
					answerfound = true;
					if (resultsre.find(ans) != resultsre.end()) {
						resultsre[ans] += 1;
					} else resultsre[ans] = 1;
