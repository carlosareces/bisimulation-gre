...
f2=open("res.txt","a")
f3=open("originalYreordered/originalYreordered"+numero+".txt","w")
...
for line in f.readlines():
	if "<TRIAL" in line and 's="'+numero+'"' in line:
		...
		if "</TRIAL>" in line and in_trial:
			...
			elif ("<ORIGINAL>" in line) or ("<REORDERED_PATTERN>" in line):
				if "<REORDERED_PATTERN>" in line:
					f3.write(line.replace("\t\t<REORDERED_PATTERN>","").replace("</REORDERED_PATTERN>",""))
