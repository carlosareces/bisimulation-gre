...
For that reason, for example, the value of \emph{bal}.\puse\ is 1, while the value of \emph{cube}.\puse is 0.178.  These probabilities will not be useful to describe different targets in different scenes.  We will see how we can use them to obtain values for new targets and scenes in the next section.  Perhaps not surprisingly, using these values the algorithm generates REs matching the ones found in the corpus.  More interestingly, as we discuss in Section~\ref{sec:evaluation} it does so with a distribution that matches the one found in the corpus and, as Table~\ref{} shows, even the generated REs not found in the corpus are natural.    
