...
is indeed what happens in the GRE3D7 corpus.  As we can see in Table~\ref{corpus-distribution}, 
the target is described 16.43\% of the times as a ``small green ball'' when ``green 
ball'' is already an RE.  Using the \puse\ values learned from corpora explained
in the previous section, Algorithm~\ref{algo:bisim-l} cannot simulate this behavior. 

Because the fundamental idea of the algorithm is semantic, handling overspecification in 
...
obtained so far, and hence the algorithm will not include it in the generated descriptions. 
On the other hand, if we disregard the condition the informativeness constraint (i.e., 
...
As a compromise, we consider the following variation of Algorithm~\ref{algo:bisim-l} where 
we disregard the informativeness constrain for propositional information (i.e., we allow the inclusion of new properties 
...
input list Rs, we will allow the inclusion of all properties that do not trivialize the 
...
In the remaining loops, additional relations will be added only if they are informative. 
