...
(2) Consejo Nacional de Investigaciones Científicas y Técnicas, Argentina\\ 
  \texttt{\small ialtamir@famaf.unc.edu.ar, carlos.areces@gmail.com, luciana.benotti@gmail.com} \\ 
}}

\begin{document}
\maketitle
%% The first mandatory ABSTRACT (\abstractEn) section below is for the English language
\abstractEn{We propose an algorithm for the generation of referring expressions that adapts the approach of~\cite{arec2:2008:Areces,arec:usin11} 
to include overspecification and probabilities learned from corpora.  After introducing the algorithm we discuss how the probabilities required as 
input can be computed for any given domain for which a suitable corpus of REs is available, and how the probabilities can be adjusted for new scenes in the domain using a machine learning approach.  We exemplify how to compute the 
