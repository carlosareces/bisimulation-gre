...
\While(\tcp*[f]{\footnotesize $R.\puse$ are incremented till they reach 1}){$\exists ((R,R.\puse) \in Rs).(R.\puse < 1)$}{
  \While(\tcp*[f]{\footnotesize while some class has at least two elements}){$\exists \varphi \in \RE. |\interp{\varphi}|>1$}{
      \RE' $\leftarrow$ \RE \tcp*[f]{\footnotesize make a copy for future comparison} \;
      \For{$(R, R.\puse) \in Rs$}{
          \If(\tcp*[f]{\footnotesize $R$ will be used in the expression}){$R.\randomuse \le R.\puse$}{
              \For{$\varphi \in \RE$}{
                  add$_\mathcal{L}(R, \varphi, \RE)$\tcp*[f]{\footnotesize refine all classes using $R$}}
                  }\;
              \If(\tcp*[f]{\footnotesize the classification has changed}){\RE $\not =$ \RE'}{exit\tcp*[f]{\footnotesize exit for-loop to try again highest $R.\puse$}}
              }
     \If({\tcp*[f]{\footnotesize the classification has stabilized})\RE $=$ \RE'}{exit\tcp*[f]{\footnotesize exit while-loop to increase $R.\puse$}}
  }
  \For{$(R,R.\puse) \in Rs$}{
    $R.\puse \leftarrow R.\puse + R.\incuse$\tcp*[f]{\footnotesize increse $R.\puse$}
  }
}
\end{algorithm}
\caption{Main algorithm, dealing with probabilities}\label{fig:algo1}
\end{figure}
...
\For{$\psi \in \RE$ with $|\interp{\psi}| > 1$}{
  \If(\tcp*[f]{\footnotesize the new class is smaller than the original}){$\psi \sqcap \exists R.\varphi$ is not subsumed in $\RE$ {\bf and} \tcp*[f]{\footnotesize the new class is not redundant}\\
    \ \ \ $\interp{\psi \sqcap \exists R.\varphi} \neq \emptyset$ {\bf and} \tcp*[f]{\footnotesize the new class is not empty}\\
    \ \ \ $\interp{\psi \sqcap \exists R.\varphi} \neq \interp{\psi}$ }{
    add $\psi \sqcap \exists R.\varphi$ to $\RE$ \tcp*[f]{\footnotesize add the new class to the classification}
    remove subsumed formulas from $\RE$ \tcp*[f]{\footnotesize remove redundant classes}
  }
}
\end{algorithm}
\caption{Refinement function for the \el-language, }\label{fig:algo2}
\end{figure}
