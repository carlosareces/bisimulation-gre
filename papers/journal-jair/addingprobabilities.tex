...
T is the most general description (a formula) which interpretation includes all elements of the model, it is like ``thing'' every element will be a thing.

We will say that a formula is subsumed for others, if their extension can be cover by the union of extensions of other formulas. For example, in the Figure~\ref{figure22}, T is subsumed to ball and cube, because $\interp{T}$ = $\{e_1, e_2, e_3, e_4, e_5, e_6, e_7\}$ and the $\interp{ball}$ = $\{e_1, e_3, e_5\}$, $\interp{cube}$ = $\{e_2, e_4, e_6, e_7\}$, it is $\{e_1, e_2, e_3, e_4, e_5, e_6, e_7\}$ = $\{e_1, e_3, e_5\} \cup \{e_2, e_4, e_6, e_7\}$. Intuitively the formula cube or ball have more information than T, for each element of T, there is a formula that gives more information, say ``cube'' is more informative than say ``thing''.\\

In the following part we will explain an example of execusion of the algorithms~\ref{algoritmoOriginal} taking into account the $\el$ logic language and then we will show how to add probabilities to the algorithms.
...
We will run the algorithm for the Figure~\ref{figure22}, the algorithm start with a fixed list of properties and relations, supose that those lists are the following:
...
\end{center}
\vspace*{-2em}
\caption{A figure of the GRE3D7 corpus - The name of the objects are not appearing in the corpus}
\label{figure22}
\end{figure}

We will return RE, there will be a formula for each element in the domain (if that formula exists or that model).\\

In the begining RE=$\{T\}$ and its $\interp{T}$ = $\{e_1, e_2, e_3, e_4, e_5, e_6, e_7\}$\\
