...
y un sistema de di\'alogo de un robot que ensambla piezas de juguetes, junto con un usuario humano~\cite{foster-etal-ijcai2009} hace referencia a los componentes --- {\it inserte el perno verde hasta el final en el cubo rojo}.

Una {\bf expresi\'on referencial (ER)} es un sintagna nominal que identifica un\'ivocamente a un objeto en un contexto y para un interlocutor particular. Si quisieramos referirnos al objeto se\~nalado por la flecha en la Figura~\ref{figura-er} podr\'iamos hacerlo con alguna de las expresiones referenciales que se muestran en la misma figura.% \ref{er-figura1}.
...
\vspace*{0.4cm}
 ...
 \label{er-figura1}\ref{GRE3D7-stimulus1}
%\end{table}
\end{minipage}
...
Dado un contexto, un target y una descripci\'on (parcial) del target, los {\bf distractores} son otros elementos que se encuentran en el contexto, y que tambi\'en cumplen con la descripci\'on parcial. Si la descripci\'on parcial es {\it esfera} las esferas que no son el target de la Figura \ref{GRE3D7-stimulus1} distractores, y por ello es necesario seguir agregando propiedades o relaciones.
...
Una computadora que se enfrenta a la tarea de generar autom\'aticamente expresiones referenciales en un contexto determinado, necesitar\'a una representaci\'on de todos los objetos y las propiedades de cada uno de ellos. En la Figura~\ref{formulas-stimulus1} se muestra una posible forma de representar los objetos, sus propiedades y relaciones. La representaci\'on ilustrada en la Figura~\ref{GRE3D7-stimulus2} es equivalente a la mostrada en la Tabla \ref{tabla-propiedades}, una base de datos que contiene todas
...
%\end{table}
\end{minipage}
\caption{Ejemplos de ER en \EL para la Figura \ref{GRE3D7-stimulus1-b}}
...
%\put(0,-50){\begin{tikzpicture}
 ...
 %\draw [aArrow,bend right=40] (g) to node[auto,swap]{\relsize{-3}$\nRightof$} (f);
 %%\draw[dotted] (-0.5,-1.3) rectangle (8,3.1);
 %\draw[dotted] (-0.5,-1.5) rectangle (8,3.3);
 %\end{tikzpicture}}
 %\end{picture}
 %%\end{flushleft}
%\vspace*{1.75cm}
...
%\hline
%%Figure & Model \puse &  Learning \puse & Random \puse &  Uniform \puse \\
%Objeto& 	Tipo		&	Color	&	Tama\~no & Al lado de & Arriba de	& Abajo de	\\
%\hline
%$e_1$ & ball & yellow & small & - & - & -\\
%$e_2$ & cube & red & small & - & - & $e_3$\\
%$e_3$ & ball & yellow & small & - & $e_2$ & -\\
...
En estos dominios, e
incluso en contexto tan simples como el de la Figura \ref{GRE3D7-stimulus1} hay incertidumbre. '?Estamos seguros que la Figura \ref{formulas-stimulus1} representa toda la informaci\'on del contexto? Algunos podemos opinar que s\'i, otros que no. En efecto, la persona que gener\'o la ER \textbf{la pelota a la izquierda del cubo amarillo} opina que no: la relaci\'on \emph{a la izquierda de} discutiblemente existente entre el target y el cubo amarillo no esta representada en el grafo. Un algoritmo de GER con este grafo como input nunca generar\'ia la expresi\'on referencial de la que venimos hablando. Esta falta de informaci\'on no s\'olo impide la generaci\'on de ER v\'alidas, si no que tambi\'en permitir\'ia la generaci\'on de ER claramente incorrectas como, \emph{la esfera roja que no tiene nada a la derecha}.
Y aunque no hablemos ni de incompletitud ni de incorrectitud tambi\'en hay incertidumbre en \textbf{cu\'al} ER un sistema de GER de elegir. Si tuvieramos que elegir la mejor ER para la Figura \ref{GRE3D7-stimulus1}, c\'omo lo har\'iamos? Se podr\'ia argumentar que tendr\'iamos tantas respuestas distintas como el n\'umero de personas a las cuales les preguntaramos. Y parte de raz\'on hay en eso. Por ejemplo seguramente hay personas que preferir\'an usar la palabra esfera o pelota, antes que bola. Para gustos hay colores. Y seguramente no falte quien diga que LA mejor expresi\'on referencial ni si quiera aparece en la Figura \ref{GRE3D7-stimulus1}. Sin embargo, seguramente habr\'a una tendencia como se muestra en estudios previos \cite{viet:gene11}. Nadie preferir\'a una ER que use 21 propiedades y 6 relaciones para describir al target de la Figura \ref{GRE3D7-stimulus1}. El largo de una ER affecta su calidad.
...
Para poder dar ese ranking del cual estuvimos hablando bastar\'ia correr un algoritmo {\bf no-determin\'istico} una cantidad 'suficiente'{\footnotesize depender\'a del grado de detalle que queremos conseguir} de veces.
