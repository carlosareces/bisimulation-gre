...
$c$ y $e$ son gatos;  $d$ es un beagle peque\~no; $b$ y $c$ son tambi\'en peque\~nos.
 Leeremos $\aSniffing(d,e)$ como ``{\em $d$ huele a $e$}''. La interpretaci\'on del s\'imbolo relacional $\nDog$ es $\interp{\nDog}$  =  $\cset{a,b,d}$ ya que $a$, $b$ y $d$ son los \'unicos perros del contexto. La interpretaci\'on de $\aSniffing$ es el conjunto de pares de elementos que cumplen $\aSniffing$. Por ejemplo ($a,a$) pertenece al conjunto dado que $a$ es un perro que se huele a si mismo en el modelo.
...
Ahora nos enfocaremos en conseguir ERs para identificar a targets dados usando los modelos que acabamos de describir. Veremos lenguajes de distintas l\'ogicas y como las f\'ormulas l\'ogicas pueden representar las ERs.
...
$$
  \item \top Es como decir \it{cosa}, todo elemento satisface \top.
	...
	\item \exists x_i . \gamma Existencial de una variable ligada en la f\'ormula de \FOL \gamma.
$$
%
...
\item $\interp{\exists x_{l}.\delta}^n$ $=$ $\cset{\bar a \mid \bar a  e  \in \interp{\delta'}^{n+1}\ \text{for some $e$}}$
...
Como ya discutimos diferentes ERs
...
Como nuestra meta es hacer un ranking de las mejores expresiones referenciales restringiendo la l\'ogica, nos podemos acercar m\'as a la meta. Si restringimos a \EPFOL, aseguramos que f\'ormulas como $\phi_2$ no se generar\'an. Si sacaramos el operador distinto ($\not\approx$) del lenguaje, $\phi_3$ tambi\'en queda exclu\'ida.
...
Para el segundo ejemplo, supongamos que queremos identificar a $e_5$ de la Figura \ref{grafo-GRE3D7-stimulus_b}. Algunas f\'ormulas posibles que identifican a un\'ivocamente a $e_5$ en el modelo se muestran en la Tabla \ref{tab:phis}.
...
$$
\begin{array}{cl}
 \phi_1: & Esfera roja\\[3pt]
  %
 \phi_2: & Esfera grande\\[3pt]
  %
 \phi_3: & Esfera grande y roja\\[3pt]
  %
 \phi_4: & Esfera grande y roja que est\'a a la derecha de un cubo grande rojo\\[3pt]
  %ex1
 \phi_5: & Esfera grande y roja \land
  \forall y . (\neg \nBall(y) \lor \neg \nRightof(x,y))\\[3pt]
  %ex 2
 \phi_6: & Esfera grande y roja \land
  \exists y . (x \not\approx y \land \nCube(y) \land \nRightof(x,y))\\[3pt]
  %ex 3
 \phi_7: & Esfera grande y roja que tiene un cubo rojo a la izquierda
  %ex 4
 \end{array}
$$
\caption{Posibles realizaciones de las f\'ormulas dadas en \protect\ref{tab:phis}}\label{tab:realizaciones-phis}
\end{table}
