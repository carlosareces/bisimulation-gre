...
\label{input_algo}
El sistema toma como input el contexto tambi\'en llamado modelo y una lista de probabilidades de las propiedades o relaciones de la signatura del modelo.
...
Desde ahora hablaremos de f\'ormulas (una f\'ormula corresponde a una descripci\'on) por ejemplo usaremos la f\'ormula ``pelota'' para referirnos a todas las pelotas del contexto, por ejemplo en el contexto \ref{grafo-GRE3D7-stimulus} ser\'ian: $e_1$, $e_3$ y $e_5$  asi como ``pelota y amarillo'' ser\'ian: $e_1$ y $e_3$, para todas las pelotas amarillas del contexto.
...
Una f\'ormula es Informativa cuando tiene m\'as informaci\'on que las f\'ormulas que hay hasta el momento, es decir, el subgrupo de objetos que la satisfacen esta inclu\'ido en alguna otra f\'ormula y no es igual. Por ejemplo si tenemos las f\'ormulas ``pelota'' y ``cubo'' y queremos saber si ``pelota roja'' es informativa, si lo es ya que $e_5$ es una ``pelota roja'' y existen otras que no son rojas, es decir divide el conjunto de pelotas. 

Una f\'ormula es subsumida si ya existen subf\'ormulas con m\'as informaci\'on que cubren todo el conjunto de objetos que la formula cubr\'ia. Por ejemplo, T que es la f\'ormula en la cual todos los objetos pertenecen, ser\'a subsumida cuando se agreguen las f\'ormulas ``pelota'' y ``cubo'' ya que todos los elementos del contexto \ref{grafo-GRE3D7-stimulus} son o pelotas o cubos, es decir ya tenemos informaci\'on m\'as precisa de los objetos.

Redundante es una f\'ormula para la cual ya hay otra f\'ormula la cual tenga los mismos objetos del modelo. Por ejemplo para el contexto \ref{grafo-GRE3D7-stimulus} si ya tenemos la f\'ormula ``pelota roja'', la f\'ormula ``pelota roja grande'' es redundante ya que no agrega informaci\'on porque el conjunto de objetos pelotas rojas es solamente 1 objeto.

Trivial es una f\'ormula para la cual no hay objetos que la satisfagan en el modelo considerado, por ejemplo en el Contexto \ref{grafo-GRE3D7-stimulus} ``pelota amarilla grande'' es trivial ya que no hay pelotas amarillas grandes en el contexto considerado.
...
\For{\em (R,R.\puse) $\in$ Rs}{
	R.\randomuse = Random(0,1)\tcp*[f]{\footnotesize R.\randomuse es la probabilidad de usar R} \;
        R.\incuse = (1 $-$ R.\puse) / MaxIterations\tcp*[f]{\footnotesize R.\puse\ son incrementadas por R.\incuse en cada ciclo}
}

\Repeat{\em $\forall$((R,R.\puse) $\in$ Rs).(R.\puse $\ge$ 1)\tcp*[f]{\footnotesize R.\puse\ son incrementadas hasta que alcanzan 1}}{
...
\For{\em $\psi \in$ \RE con $\#\interp{\psi} > 1$}{
  \If{\em $\psi \sqcap \exists$R.$\varphi$ no esta subsumida en \RE\ {\bf y} \tcp*[f]{\footnotesize es no-redundante: no puede ser obtenida desde \RE?}\\
...
\hline
\end{tabular}
\caption{Probabilidades de uso de las palabras del corpus en Table~\ref{GRE3D7-stimulus-4} para las Figuras 3 y 13 del \textit{(GRE3D7)} 
