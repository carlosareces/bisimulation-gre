...
La salida del algoritmo es un conjunto de f\'ormulas (que ser\'an expresiones referenciales si la clase de la f\'ormula contiene s\'olo 1 objeto) de los elementos del modelo. Si cada f\'ormula tiene un solo objeto, tenemos una ER para cada elemento del modelo, es decir el algoritmo da ER para todos los objetos que puede identificar un\'ivocamente. 
...
Por ejemplo, existe la interpretaci\'on de la f\'ormula \emph{esfera}$\sqcap$ \emph{a-la-izquierda-de}. \emph{cubo} es el conjunto de todas las esferas que est\'an a la izquierda de alg\'un cubo.
...
con esto nos aseguramos que en MaxIterations todas lleguen a 1, y en consecuencia el algoritmo termine, ya que la l\'inea 15 del c\'odigo incremente R.puse con R.incuse y la 16 dice salir del ciclo principal si para todas las R, R.puse es >=1

\subsection{Generando ER no-determ\'inisticamente}

Al iniciar el algoritmo (en l\'inea 3) calcula para cada R, R.rnduse que es un n\'umero aleatorio entre 0 y 1, ese n\'umero va a hacer que el algoritmo sea no-determin\'istico, ya que en la l\'inea 9, el algoritmo usara R para refinar las clases solamente si 
...
%the target is described 16.43\% of the times as a ``peque\~no green esfera'' when ``green 
%esfera'' is already an RE.  Using the \puse\ values learnt from the calculus explained
...
%%\newcommand{\npeque\~no}{\mathit{peque\~no}\xspace}
%%\newcommand{\nBig}{\mathit{big}\xspace}
%%\newcommand{\nesfera}{\mathit{esfera}\xspace}
%%\newcommand{\ncubo}{\mathit{cubo}\xspace}
...
%      \nBig\\[-2pt] 
%      \nBlue \ \  
%      \ncubo\end{array}$}, right of=a] (b) {};

% \node[n,label=below:$e_3$,label=above:{
%    \relsize{-1}$\begin{array}{c}
%      \nTop \ \       \nLeft \ \ 
%      \npeque\~no \ \       \nGreen \ \ 
%      \nesfera\end{array}$}, above of=b] (c) {};

% \node[n,label=above:$e_4$,label=below:{
%    \relsize{-1}$\begin{array}{c}
%      \npeque\~no\\[-2pt] 
%      \nGreen\\[-2pt] 
%      \ncubo\end{array}$}, right of=b] (d) {};

% \node[n,label=above:$e_5$,label=below:{
%    \relsize{-1}$\begin{array}{c}
%      \nBig\\[-2pt] 
%      \nBlue\\[-2pt] 
%      \nesfera\end{array}$}, right of=d] (e) {};

% \node[n,label=above:$e_6$,label=below:{
%    \relsize{-1}$\begin{array}{c}
%      \nBig\\[-2pt] 
%      \nGreen\\[-2pt] 
%      \ncubo\end{array}$}, right of=e] (f) {};
...
%For example, the interpretation of the formula \emph{esfera} $\sqcap \exists$\emph{leftof}.\emph{cubo} is the set of all esferas that are on the left of some cubo.  
...
%\lElse(\tcp*[f]{\footnotesize informative:peque\~noer than the original?}) {Informative $\leftarrow$ $\interp{\psi \sqcap \exists \mbox{\em R}.\varphi} \neq \interp{\psi}$} 
%\For{\em $\psi \in$ \RE with $\#\interp{\psi} > 1$}{
...
%\item Remove hyperonyms from $T$. E.g., if both \emph{cubo} and
...
%  and \emph{peque\~no} are both represented by the token \emph{peque\~no}.
...
%value of \emph{esfera}.\puse\ is 1, while the value of
%\emph{cubo}.\puse\ is 0.178.  These probabilities will not be useful
...
\hline
%green esfera & 91 & 65.00\% \\
%peque\~no green esfera & 23 & 16.43\% \\
%peque\~no green esfera on top of grande blue cubo & 8 & 5.71\% \\
%green esfera on top of blue cubo & 5 & 3.57\% \\
%green esfera on top of grande blue cubo & 5 & 3.57\% \\
%peque\~no green esfera on top of blue cubo & 2 & 1.43\% \\
%esfera on top of cubo & 1 & 0.71\% \\
%peque\~no green esfera on top of grande blue cubo to the left & 1 & 0.71\% \\
%peque\~no esfera on top grande cubo & 1 & 0.71\% \\
%green esfera on top & 1 & 0.71\% \\
%peque\~no esfera on top of peque\~no cubo & 1 & 0.71\% \\
%green esfera on top of cubo & 1 & 0.71\% \\
...
%for Fig 13 the value estimated for ``grande'' are not close to the
...
%ALGO ORIGINAL MOD ejemplo para el nuestro 
%\begin{figure}[h!]
%\begin{center}
...
%
%propiedades ordenadas (prop): \textsf{esfera}, \textsf{cubo}, \textsf{red}, \textsf{yellow}, \textsf{peque\~no}, \textsf{grande}.\\
