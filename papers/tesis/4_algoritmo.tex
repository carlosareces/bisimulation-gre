...
En las siguientes secciones explicaremos el algoritmo en detalle
...
ACA: Quizas deba poner un ejemplo 
...
y una lista de probabilidades ordenadas de mayor a menor con el orden de preferencia de las propiedades y relaciones. Por ejemplo esta es una lista de probabilidades de uso de la Figura \ref{GRE3D7-stimulus}
...
Desde ahora hablaremos de {\it f\'ormulas} (una f\'ormula corresponde a una descripci\'on) por ejemplo usaremos la f\'ormula ``pelota'' para referirnos a todas las pelotas del contexto, en el contexto \ref{grafo-GRE3D7-stimulus} ser\'ian: $e_1$, $e_3$ y $e_5$  as\'i como ``pelota y amarillo'' ser\'ian: $e_1$ y $e_3$.\\

Una clase es un conjunto de objetos que satisface la misma f\'ormula (descripci\'on), por ejemplo ``pelota'' ser\'an todas las pelotas del modelo.\\

Una f\'ormula es {\it Informativa} cuando tiene m\'as informaci\'on que las f\'ormulas que hay hasta el momento, es decir, el subgrupo de objetos que la satisfacen esta inclu\'ido en alguna otra f\'ormula y no es igual. Por ejemplo si tenemos las f\'ormulas ``pelota'' y ``cubo'' y queremos saber si ``pelota roja'' es informativa, si lo es ya que $e_5$ es una ``pelota roja'' y existen otras que no son rojas, es decir divide el conjunto de pelotas. \\

Una f\'ormula es {\it subsumida} si ya existen subf\'ormulas con m\'as informaci\'on que cubren todo el conjunto de objetos que la formula cubr\'ia. Por ejemplo, T que es la f\'ormula en la cual todos los objetos pertenecen, ser\'a subsumida cuando se agreguen las f\'ormulas ``pelota'' y ``cubo'' ya que todos los elementos del contexto \ref{grafo-GRE3D7-stimulus} son o pelotas o cubos, es decir ya tenemos informaci\'on m\'as precisa de los objetos.\\

{\it Redundante} es una f\'ormula para la cual ya hay otra f\'ormula la cual tenga los mismos objetos del modelo. Por ejemplo para el contexto \ref{grafo-GRE3D7-stimulus} si ya tenemos la f\'ormula ``pelota roja'', la f\'ormula ``pelota roja grande'' es redundante ya que no agrega informaci\'on porque el conjunto de objetos pelotas rojas es solamente 1 objeto.\\

{\it Trivial} es una f\'ormula para la cual no hay objetos que la satisfagan en el modelo considerado, por ejemplo en el Contexto \ref{grafo-GRE3D7-stimulus} ``pelota amarilla grande'' es trivial ya que no hay pelotas amarillas grandes en el contexto considerado.
...
%no se entiende
Comienzo
de la escena en la Figura~\ref{GRE3D7-stimulus-4} y el correspondiente
...
\hline
\end{tabular}
\caption{Expresiones Referenciales producidas por las personas para la Figura~\ref{GRE3D7-stimulus-4}}\label{corpus-distribution}
\end{center}
\end{table}
...
(verde y azul) y (pelota y cubo). En otras palabras, en el corpus GRE3D7 se utiliza el tama\~no con m\'as frecuencia (90,2 \%)
...
en~\cite[Cap\'{i}tulo 6] {viet:gene11}, no pod\'{i}a ser aprendido por \'arboles decisi\'on. Como resultado podemos ver en la tabla~\ref{probability-of-use} de la Figura 13, el valor estimado para ``grande'' no est\'a cerca de la
...
Palabra &  \puse 					& \puse Aprendida & \puse    							& \puse  Aprendida \\
        & Modelo Fig. 3   & Fig. 3   				& Modelo Fig. 13  			&  Fig. 13 \puse \\
\hline
pelota & 1.0 & 1.0 & 1.0 & 1.0 \\
...
\hline
\end{tabular}
\caption{Probabilidades de uso de las palabras del corpus en Tabla~\ref{GRE3D7-stimulus-4} para las Figuras 3 y 13 del \textit{(GRE3D7)} 
...
WEKA~\cite{Hall:WEK09}, entrenando con todas las escenas menos una (para el que estamos aprendiendo) para todas las escenas del \textit{GRE3D7 y TUNA-corpus}. 
