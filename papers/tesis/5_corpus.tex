...
\noindent
Además de  16 atributos que denotan los objetos adicionales (landmarks) que se mencionaron en la descripci\'on, a los cuales llamamos d1..d4. Por ejemplo, en {\it El pub que est\'a al lado de Pans \& Company, en la calle de las Postas}) tenemos:target = pub, d1 = fast-food (Pans \& Company) y d2 = Street (calle de las Postas).
...
A partir de estos resultados se ve que algunos atributos (por ejemplo, el tipo del target y del landmark y el nombre del landmark) se clasificaron con gran precisi\'on, mientras que otros (por ejemplo, las relaciones) no se clasificaron con alta precisi\'on.
...
La precisi\'on que conseguimos en este caso es de 27.1\%.
%18.3+ 0.5+8.3
