...
$\+L$-GRE($\+M$, T)=\{$\phi$ en $\+L$ | $\interp{\phi}$ = T $\or$ $\botom$ \}
...
En nuestro ejemplo tenemos que $e_5 \simil{\+C} e_1$ y $e_5 \simil{\+C} e_2$. Por lo que $\+C$-GER no tiene soluci\'on, para este ejemplo es \botom. Si permitimos usar un lenguaje m\'as expresivo como $\+EL$ que permite usar relaciones binarias la f\'ormula $\nSmall} \land \exists rightof. \nCube$ es una ER para el target del ejemplo. 
