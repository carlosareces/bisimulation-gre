...
De acuerdo a la complejidad de las estructuras gramaticales que queramos permitir en las ERs, tendremos un lenguaje asociado, 
...
En esta secci\'on veremos dos conceptos importantes, similaridad en $\+L$, y simulaci\'on en $\+L$. Recordemos que en la Secci\'on \ref{sec:gre-teoria-modelos} definimos la salida del problema GER con el lenguaje $\+L$ (es decir $\+L$-GER) como una f\'ormula $\phi$ en $\+L$ cuya interpretaci\'on en el modelo de entrada  $\+M$ es el conjunto target T, si esa f\'ormula existe. Formalmente, $\+L$-GER($\+M$, T)=\{$\phi$ en $\+L$ | $\interp{\phi}$ = T\} $\lor \perp$ en caso de no existir ninguna f\'ormula.
...
\end{figure}
Ahora si buscamos una ER para el target T = \{$e_5$\} en la Figura \ref{target_mapa_3ball} usando el lenguaje $\+C$, veremos que no existe una ER en $\+C$ para $e_5$. Observando el modelo de la figura vemos que las f\'ormulas que son ciertas para $e_5$ de $\+C$ y sus interpretaciones son las siguientes:
...
bien conocidos (vea, \cite{hopc:algo71,HHK95,dovier04:_effic_algor_for_comput_bisim_equiv,areces08}), y para muchos
...
En la Tabla \ref{tab:algoritmos-complejidad} se muestra una comparaci\'on entre los algoritmos m\'as conocidos del \'area, que l\'ogica de descripci\'on (DL) usan y la complejidad.
...
Primero explicamos c\'omo computar $\simset_{\EL}^{\+M}(u)$, es decir, los conjuntos de objetos indistinguibles en \EL y luego usamos este c\'omputo para la GER.

\subsection{Computando conjuntos de objetos indistinguibles en $\EL$}
\label{sec:indistinguiblesEL}

Un algoritmo para computar $\simset_{\ELAN}^{\+M}(v)$ es dado en~\cite{HHK95}. Para todos los
elementos $v$ de un modelo dado finito
$\+M=\tup{\Delta,\interp{\cdot}}$
computa $\simset_{\ELAN}^{\+M}(v)$ en tiempo $O(\size{\Delta}\times\size{{\interp{\cdot}}})$.
...
Al final, $S(v)=\simset_\ELAN^{\+M}(v)$. El algoritmo puede ser adaptado para
computar $\simset_\+L$ para cualquier otro lenguaje $\+L$. En particular,
lo podemos usar para computar $\simset_\EL^{\+M}$ en tiempo polinomial y nos dar\'a un l\'imite superior a la
...
y $w'\in S(v)$) entonces es claro que la condici\'on $\zig$ no se cumple $S(u)$ es `demasiado grande' porque
...
El Algoritmo~\ref{alg:schematic-GRE} se puede ejecutar en un subconjunto arbitrario del dominio de $\tup{\Delta,\interp{\cdot}}$
...
A continuaci\'on daremos una introducci\'on a la l\'ogica de descripci\'on \alc y la comparamos con \el. Las f\'ormulas $\varphi$ de $\alc$ son generadas por la siguiente gram\'atica:
$$
\varphi,\varphi' ::= \top \mid p \mid \neg \varphi \mid \varphi \sqcap \varphi'
\mid \exists R. \varphi
$$
donde $p \in \prop$, es decir, es el conjunto de los s\'imbolos proposicionales y $R \in \rel$ es el conjunto de los s\'imbolos relacionales. $\el$ puede definirse como $\alc$ sin negaci\'on. $\alc$ permite negaci\'on tanto at\'omica (por ejemplo, $\neg \mathsf{flower}$) como sobre relaciones (por ejemplo, $\neg \exists \mathsf{in}.\mathsf{hat}$).
...
hay un riesgo de que el \el-algoritmo tenga peor tiempo de ejecuci\'on, como se muestra en \cite{areces08}.\\
...
$\mathsf{bowl}$ es redundante ya que est\'an $\mathsf{bowl} \sqcap
...
\EPFOL-distractores parece requerir necesariamente una soluci\'on al problema de isomorfismos de subgrafos, que es NP-completo. Mientras que en las Secciones \ref{sec:greViaSimulacion} y \ref{sec:bisimulacion} presentamos algoritmos que pueden encontrar ERs en tiempo polinomial.
%en ~ \ secta {tama\~no}.
