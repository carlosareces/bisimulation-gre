...
\begin{figure}[ht]
\centering
\includegraphics[width=0.8\textwidth]{images/22sinletras.jpg}
\caption{Ejemplo de contexto}
\label{GRE3D7-stimulus}
\end{figure}

Una propiedad es una caracter\'istica propia de un objeto. Por ejemplo en el contexto \ref{GRE3D7-stimulus} el objeto se\~nalado con la flecha tiene la propiedad color con valor rojo. Una relaci\'on caracteriza a un objeto describiendo caracter\'isticas de otro u otros objetos.\\
...
Una ER {\it proposicional} incluye s\'olo propiedades intr\'insecas del objeto target. Por ejemplo en el contexto de la imagen \ref{GRE3D7-stimulus}: ``La pelota roja''. Las caracter\'isticas de ser pelota, ser roja son propias del objeto identificado.\\

Una ER es {\it relacional}, cuando contiene relaciones con otros objetos, entonces la ER incluye las ERs de los otros objeto.  Por ejemplo: ``La pelota que esta a la derecha del cubo grande.'' ``a la derecha de'' es una relaci\'on que necesita describir en este caso al ``el cubo grande''.\\

Se dice que una ER es {\it minimal}, cuando incluye la m\'inima cantidad de propiedades con las cuales el objeto puede ser identificado un\'ivocamente en el contexto dado. Notar que puede haber muchas expresiones minimales. Por ejemplo en el contexto de la imagen \ref{GRE3D7-stimulus} ``La pelota roja'' es una ER minimal, como asi tambi\'en lo es ``La pelota grande'', ya que ambas tienen 2 propiedades del objeto target.\\

Cuando la ER contiene m\'as informaci\'on de la m\'inima necesaria para distinguirla de los dem\'as objetos en el contexto dado, se dice que la ER es {\it sobreespecificada}. Por ejemplo: ``La pelota roja grande'' en el contexto de la imagen \ref{GRE3D7-stimulus}.\\

Un expresi\'on es {\it subespecificada} cuando no alcanza a ser una ER, ya que no se puede distinguir al objeto target de todos los objetos en el contexto. Por ejemplo: ``La pelota'' en el contexto \ref{GRE3D7-stimulus}. En estos casos se debe dar otra expresi\'on o corregir la anterior agregando m\'as propiedades o relaciones. Esta expresi\'on identifica a un conjunto, no a un objeto \'unico.

\section{La tarea de generaci\'on autom\'atica de expresiones referenciales}
quizas mover a la introduccion..

La generaci\'on autom\'atica de expresiones referenciales (GER), es la tarea de definir cuales pasos se deben seguir para conseguir una ER en un contexto dado.\\



\section{Tipos de algoritmos de GER}

Si pensamos como generar autom\'aticamente ERs, hay distintas tipos de algoritmos para generarlas, un algoritmo pueden ser: determin\'{i}stico o no-determin\'{i}stico, relacional o proposicional, incluir negaciones o no, identificar plurales o s\'olo singulares, usar disyunciones y conjunciones, o s\'olo conjunciones, generar ER sobreespecificadas o minimales. Hacer o no backtraking. Los algoritmos adem\'as se pueden diferenciar por el input que toman, es decir el modo en que toman el contexto y el target para el cual se espera que den una expresi\'on referencial. A continuaci\'on se describen cada uno de esos tipos.\\
...
Un algoritmo es {\it relacional} si permite generar ER relacionales, en cuyo caso adem\'as de generar las relaciones correspondientes deber\'a generar ERs para el o los objetos con el cual el objeto target tiene las relaciones que quiere nombrar. La {\it aridad} de una relaci\'on es la cantidad de par\'ametros que dicha relaci\'on toma. Ej.: ``el restaurante que esta en la esquina de Gral. Paz y Col\'on'', la relaci\'on esquina tiene aridad 2 ya que tiene 2 calles como par\'ametros.\\

En algunos contextos cuando el target es el \'unico que no tiene una propiedad por ejemplo, ser\'ia \'util un algoritmo que incluya {\it negaciones}. Supongamos que en la im\'agen hay un conjunto de ovejas, y solo una es de otro color que no es blanco. Un algormitmo que incluye negaciones podr\'ia decir ``La que no es blanca''.\\

Un algoritmo puede generar ER para {\it plurales}, es decir dar ER para varios targets de la escena considerada. ``Los dos restaurantes de la calle principal''. \\
...
Un algoritmo incluir\'ia propiedades o relaciones en cada paso, un algoritmo que hace backtraking, es el que luego de incluir una propiedad o relaci\'on puede deshacer esa inclusi\'on volviendo a un estado anterior, es decir, decidir no incluirla.\\

Con respecto al input de una algoritmo de GER, el contexto puede venir dado en distintos lenguajes, por ejemplo: el contexto que toma como input el algoritmo GRAPH para la imagen \ref{GRE3D7-stimulus} es un grafo etiquetado como se muestra en \ref{grafo-GRE3D7-stimulus}, el cual contiene la informac\'on de las propiedades y relaciones de los objetos, y el contexto para el algoritmo de bisimulaci\'on es un XML con la misma informaci\'on como se muestra en \ref{input-bisimulacion}.
...
\begin{tikzpicture}
 ...
 \node[n,label=below:$e_7$,label=below:{
    \relsize{-1}$\begin{array}{c}
      \nSmall\\[-2pt]
      \nRed\\[-2pt] 
...
<problem refer-to="s1">c
  ...
  <individual id="s2">
    ...
    <related rel="small" to="c" />
    <related rel="above-of" to="s3" />
  </individual>
  <individual id="s3">
    <related rel="left" to="c" />
    <related rel="ball" to="c" />
    <related rel="yellow" to="c" />
    <related rel="small" to="c" />
    <related rel="on-top" to="s2" />
  </individual>
  <individual id="s4">
    <related rel="cube" to="c" />
    <related rel="red" to="c" />
    <related rel="large" to="c" />
    <related rel="left-of" to="s5" />
  </individual>
  <individual id="s5">
    <related rel="large" to="c"/>
    <related rel="ball" to="c" />
    <related rel="red" to="c" />
    <related rel="right-of" to="s4" />
  </individual>
  <individual id="s6">
    <related rel="small" to="c" />
    <related rel="cube" to="c" />
    <related rel="yellow" to="c" />
    <related rel="left-of" to="s7" />
  </individual>
  <individual id="s7">
    <related rel="small" to="c" />
    <related rel="cube" to="c" />
    <related rel="red" to="c" />
    <related rel="right-of" to="s6" />
  </individual>
  <individual id="c">
...
\section{Algoritmos del estado del arte en el \'area}
...
El algoritmo {\it Full Brevity} (Dale 1989) genera la m\'as corta descripci\'on que identifica al target. Para hacerlo, 
...
El algoritmo de heur\'istica Greedy es m\'as eficiente que el Full Brevity, pero pronto fueron superados por el algoritmo {\it Incremental} (Reiter  and  Dale  1992;  Dale  and  Reiter  1995), que fue y sigue siendo uno de los algoritmos m\'as importantes del \'area, el cual se explica a continuaci\'on. 
...
\KwIn{\footnotesize \{r\}, D, Pref}
\KwOut{\footnotesize L}

$\ L \leftarrow \emptyset $\tcp*[f]{\footnotesize the most general description $\top$ applies to all elements in the scene}\\
$\ C \leftarrow D - \{r\} $\tcp*[f]{\footnotesize the most general description $\top$ applies to all elements in the scene}\\
\For{\em (A_{i}) $\in$ Pref}{
	$\ V = Value(r,A_{i}) $\tcp*[f]{\footnotesize R.\randomuse is the probability of using R} \;
       
%     \If(\tcp*[f]{\footnotesize the classification has stabilized}){\em \C $\cap$ \RulesOut(\[A_{i}\],V) $=$ \emptyset}
	%		{then\tcp*[f]{$\ L \leftarrow \emptyset $\tcp*[f]{\footnotesize the most general description $\top$ applies to all elements in the scene}}}
		%	{else\tcp*[f]{$\ C \leftarrow \C $-$ \RulesOut(\[A_{i}\],V) }}
		%	\If(\tcp*[f]{\footnotesize the classification has stabilized}){\em \C $=$ \emptyset}{then\tcp*[f]{$\ Return L {\footnotesize the most general description $\top$ applies to all elements in the scene}}}
}
\end{algorithm}
\end{figure}
...
En este cap\'itulo daremos una introducci\'on a las l\'ogicas de descripci\'on (DL del acr\'onimo en ingl\'es description logic) %y luego daremos el algoritmo en el cual nos basamos en esta tesis.\\ el algoritmo en el cap 4

La idea es transformar el problema de GER al problema de computar una f\'ormula de DL cuya extensi\'on es el elemento target (o los elementos targets, en el caso de querer dar ER para plurales) aprovechando que una f\'ormula describe un conjunto que puede contener m\'as de un elemento.\\
