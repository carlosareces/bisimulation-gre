...
Una ER es {\it relacional}, cuando contiene relaciones con otros objetos, entonces la ER incluye las ERs de los otros objeto.  Por ejemplo: ``La pelota que esta a la derecha del cubo grande.'' ``a la derecha de'' es una relaci\'on que necesita describir otro objeto, en este caso al ``el cubo grande''.\\
...
Un expresi\'on es {\it subespecificada} cuando no alcanza a ser una ER, ya que no se puede distinguir al objeto target de todos los objetos en el contexto. Por ejemplo: ``La pelota'' en el contexto \ref{GRE3D7-stimulus} no alcanza para distinguir el objeto apuntado por la flecha, ya que hay otras 2. En estos casos se debe dar otra expresi\'on o corregir la anterior agregando m\'as propiedades o relaciones. Esta expresi\'on identifica a un conjunto, no a un objeto \'unico.
...
Acontinuaci\'on damos 2 ejemplos de diferentes maneras de modelar el contexto.\\
...
\begin{tikzpicture}
 ...
 \draw[dotted] (-.5,-1.4) rectangle (7.5,3.1);
...
generar ER sobreespecificadas o minimales. Hacer o no backtraking.  A continuaci\'on se describen cada uno de esos tipos.\\
...
Un algoritmo puede generar ER para {\it plurales}, es decir dar ER para varios targets de la escena considerada. Por ejemplo para el Contexto \ref{plurales},``Los objetos grandes''. En este caso el target no es \'unico, sino un conjunto de objetos. \\
...
La sobreespecificaci\'on es la caracter\'istica de dar m\'as atributos o relaciones de las necesarias para identificar al objeto target. Por ejemplo para el Contexto \ref{GRE3D7-stimulus-conLetras} la ER ``La pelota roja, grande que esta a la derecha del cubo rojo grande''.\\
...
L inicialmente es la descripci\'on vac\'{i}a, al finalizar la ejecusi\'on tendr\'a el conjunto de propiedades con los cuales identificaremos a r, es decir una expresi\'on referencial de r.\\
