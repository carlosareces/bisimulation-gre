...
Algorithm~\ref{algo:bisim-l} takes as input a model and a list Rs of pairs (R,R.\puse) that links each relation R $\in \REL$, the set of all relation symbols in the model\footnote{We represent each unary relation R as binary, hence $\interp{\exists \mbox{\em R}.\varphi$} is the set of all elements in the model that have the property R},  to some probability of use R.\puse. For example, \emph{green} and \emph{large} are relations in the model of Figure~\ref{TUNA-scene-graph}.  
