...
In the aplication we will use the language EL because is more natural for RE, 
% , we only need to know that the interpretation of the formula $\psi \sqcap \exists$R.$\varphi$ is the set of all elements that satisfy $\psi$ and that are related by relation R to some element that satisfy $\varphi$. For example, the interpretation of the formula \emph{ball} $\sqcap \exists$\emph{leftof}.\emph{cube} is the set of all balls that are on the left of some cube.  


%We need first to introduce some basic notions.  
...
We call Fii subsumed if the union of another Fij if the union of interpretations of Fij s is the interp of Fii, so Fii have not information enough, it means we have a formula with more information for the elements of the interp of Fii\\
...
ordered properties (prop): ball, cube, red, yellow, small, red.\\
...
\end{center}
\vspace*{-2em}
\caption{A figure of the GRE3D7 corpus\label{figure22}}
\end{figure}

We will return RE, that will compute formula for all elements in the domain.\\

RE=T=$\{ e_1, e_2, e_3, e_4, e_5, e_6, e_7\}$\\ in the beginig 
The first for divides the T set in sets of  each property
Let's start with the property ball, so the algorithm has the set of balls: $\{ e_1, e_3, e_5 \}$, cubes: $\{ e_2, e_4, e_6, e_7 \}$, now we are capable to delete the T, because it is subsumed to the two other sets. 
red things: $\{e_5\}$, $\{e_2, e_4, e_7\}$\\
